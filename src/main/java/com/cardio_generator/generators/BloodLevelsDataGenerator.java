...
public class BloodLevelsDataGenerator implements PatientDataGenerator {
  ...
  public void generate(int patientId, OutputStrategy outputStrategy) {
    try {
      ...
      double cholesterol =
          baselineCholesterol[patientId] + (random.nextDouble() - 0.5) * 10; // Small variation
      double whiteCells =
          baselineWhiteCells[patientId] + (random.nextDouble() - 0.5) * 1; // Small variation
      double redCells =
          baselineRedCells[patientId] + (random.nextDouble() - 0.5) * 0.2; // Small variation
